 ...
 * @author PC
 */
@WebServlet(name = "SearchByPriceServlet", urlPatterns = {"/searchbyprice"})
public class SearchByPriceServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (num == 1) {
            list = pd.searchByPrice(1000000, 100000);
            request.setAttribute("product", list);
            request.getRequestDispatcher("shop.jsp").forward(request, response);
        } else if (num == 2) {
            list = pd.searchByPrice(10000000, 1000001);
            request.setAttribute("product", list);
            request.getRequestDispatcher("shop.jsp").forward(request, response);
        } else {
            list = pd.searchByPrice(100000000, 10000000);
            request.setAttribute("product", list);
            request.getRequestDispatcher("shop.jsp").forward(request, response);
        }
    }
